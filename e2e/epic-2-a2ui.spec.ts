...
test.describe("Epic 2: A2UI Integration", () => {
  ...
  test("A2UI components render in chat responses", async ({ page }) => {
    ...
    // Wait for response
    await page.waitForSelector("[data-a2ui], [data-testid='a2ui-component']", { timeout: 30000 });
  ...
  test("PortfolioSummaryCard renders with correct data", async ({ page }) => {
    ...
    await page.waitForSelector("[data-a2ui='PortfolioSummaryCard'], [data-testid='portfolio-summary']", {
      timeout: 30000,
    });

    // Card should show value (CHF amount)
    const card = page.locator("[data-a2ui='PortfolioSummaryCard'], [data-testid='portfolio-summary']");
  ...
  test("ESGScoreGauge renders score visually", async ({ page }) => {
    ...
    // Wait for ESG gauge
    await page.waitForSelector("[data-a2ui='ESGScoreGauge'], [data-testid='esg-gauge'], [data-testid='esg-score']", {
      timeout: 30000,
    });

    // Should show score number
    const gauge = page.locator("[data-a2ui='ESGScoreGauge'], [data-testid='esg-gauge'], [data-testid='esg-score']");
  ...
  test("A2UI JSON in response is parsed and rendered", async ({ page }) => {
    ...
    await page.waitForSelector("[data-a2ui], [data-testid='holdings-list']", { timeout: 30000 });
  ...
  test("invalid A2UI falls back to plain text", async ({ page }) => {
    ...
    await page.waitForSelector("[data-testid='chat-message'], .chat-message, [data-testid='assistant-message']", {
      timeout: 30000,
    });
  ...
  test("A2UI buttons trigger actions", async ({ page }) => {
    ...
    await page.waitForSelector("[data-a2ui='ActionButton'], [data-a2ui-action], button[data-action]", {
      timeout: 30000,
    });

    // Click an action button
    const actionButton = page.locator("[data-a2ui='ActionButton'], [data-a2ui-action], button[data-action]").first();
    await actionButton.click();

    // Should trigger some response (new message or state change)
    await page.waitForTimeout(1000);
    const messageCount = await page.locator("[data-testid='chat-message'], .chat-message").count();
  ...
  test("A2UI data changes sync with app state", async ({ page }) => {
    ...
    const initialValue = await page.locator("[data-testid='portfolio-value']").textContent().catch(() => null);
