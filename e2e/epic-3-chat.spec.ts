...
test.describe("Epic 3: AI Chat Advisor", () => {
  ...
  test("chat sends message and receives response", async ({ page }) => {
    ...
    await expect(
      page.locator("[data-testid='user-message'], .user-message").last(),
    ).toContainText("Hello");
  ...
  test("advisor uses professional financial language", async ({ page }) => {
    ...
    const hasFinancialTerm = financialTerms.some((term) =>
      term.test(text || ""),
    );
  ...
  test("conversation maintains context", async ({ page }) => {
    ...
    const maintainsContext = contextPatterns.some((pattern) =>
  ...
  test("advisor generates A2UI components for data queries", async ({
    page,
  }) => {
    const chatInput = page
      .getByTestId("chat-input")
    ...
    const structuredContent = page.locator(
      "table, [role='table'], [data-testid='card']",
    );
