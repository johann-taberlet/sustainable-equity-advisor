...
test.describe("Epic 7: ESG Screening", () => {
  ...
  test("natural language screening via chat", async ({ page }) => {
    ...
    await chatInput.fill(
      "Show me European tech stocks with ESG score above 70",
    );
    await sendButton.click();

    await page.waitForSelector(
      "[data-testid='assistant-message'], .assistant-message, [data-a2ui]",
      {
        timeout: 60000,
      },
    );
    ...
    const mentionsStocks = /stock|company|companies|result|found/i.test(
      text || "",
    );
  ...
  test("sector filter is available", async ({ page }) => {
    ...
    await page.waitForSelector(
      "[data-testid='screening-content'], [data-testid='sector-filter']",
      {
        timeout: 10000,
      },
    );
  ...
  test("filtering by sector returns relevant results", async ({ page }) => {
    ...
    expect(text?.toLowerCase()).toMatch(
      /healthcare|pharma|medical|novartis|johnson|pfizer/i,
    );
  });
  ...
  test("can filter by minimum ESG score", async ({ page }) => {
    ...
    expect(text?.toLowerCase()).toMatch(
      /80|above|threshold|score|esg|high|top/i,
    );
  });
  ...
  test("ESG threshold filter excludes low scores", async ({ page }) => {
    ...
    await chatInput.fill(
      "Only show companies with excellent ESG ratings above 85",
    );
    await sendButton.click();
  ...
  test("screening results handle multiple items", async ({ page }) => {
    ...
    const hasPagination = page.locator(
      "[data-testid='pagination'], [aria-label*='page'], .pagination",
    );
    const hasMultipleResults =
      /\d+\s*(results|companies|stocks|found)|showing|page/i.test(text || "");
    const hasLoadMore = page.locator(
      "button:has-text('more'), button:has-text('next')",
    );
    ...
    expect(
      hasMultipleResults || paginationCount > 0 || loadMoreCount > 0,
    ).toBeTruthy();
  });
